...
class SamlResponseValidationTests {
    ...
    private static final String AUTHN_REQUEST_ID = "_acmehr-request-123";
    ...
    private static final X509Certificate TEST_CERTIFICATE;
    private static final PrivateKey TEST_PRIVATE_KEY;

    static {
        OpenSamlInitializationService.initialize();
        TEST_CERTIFICATE = readCertificate(TEST_CERTIFICATE_PEM);
    ...
    void wrongAudienceIsRejected() {
        ...
        assertRejected(response, Saml2ErrorCodes.INVALID_ASSERTION);
    }

    @Test
    void wrongDestinationIsRejected() {
        Response response = validResponse();
        response.setDestination("http://localhost:8080/saml/acs");

        Saml2AuthenticationException error = assertRejected(response, Saml2ErrorCodes.INVALID_DESTINATION);
        assertThat(error.getSaml2Error().getDescription()).contains("http://localhost:8080/saml/acs");
    }

    @Test
    void wrongRecipientIsRejected() {
        Response response = validResponse();
        confirmationData(response).setRecipient("http://localhost:8080/saml/acs");

        assertRejected(response, Saml2ErrorCodes.INVALID_ASSERTION);
    }

    @Test
    void wrongResponseInResponseToIsRejected() {
        Response response = validResponse();
        response.setInResponseTo("_different-request");

        assertRejected(response, Saml2ErrorCodes.INVALID_IN_RESPONSE_TO);
    }

    @Test
    void wrongConfirmationInResponseToIsRejected() {
        Response response = validResponse();
        confirmationData(response).setInResponseTo("_different-request");

        assertRejected(response, Saml2ErrorCodes.INVALID_ASSERTION);
    }

    @Test
    void notYetValidAssertionIsRejected() {
        Response response = validResponse();
        Instant now = Instant.now();
        response.getAssertions().get(0).getConditions().setNotBefore(now.plus(Duration.ofMinutes(2)));

        assertRejected(response, Saml2ErrorCodes.INVALID_ASSERTION);
    }

    @Test
    void expiredAssertionConditionsAreRejected() {
        Response response = validResponse();
        Instant now = Instant.now();
        Conditions conditions = response.getAssertions().get(0).getConditions();
        conditions.setNotBefore(now.minus(Duration.ofMinutes(5)));
        conditions.setNotOnOrAfter(now.minus(Duration.ofMinutes(2)));

        assertRejected(response, Saml2ErrorCodes.INVALID_ASSERTION);
    }

    @Test
    void expiredBearerConfirmationIsRejected() {
        Response response = validResponse();
        confirmationData(response).setNotOnOrAfter(Instant.now().minus(Duration.ofMinutes(2)));
    ...
    private static Assertion validAssertion(Instant now) {
        ...
        authnStatement.setSessionIndex("_session-123");
    ...
    private static Audience audience(Response response) {
    ...
    private static SubjectConfirmationData confirmationData(Response response) {
        return response.getAssertions()
                .get(0)
                .getSubject()
    ...
    private static RelyingPartyRegistration relyingPartyRegistration() {
        Saml2X509Credential verification = Saml2X509Credential.verification(TEST_CERTIFICATE);
